...
import { ProfileTopic, Selected, SubTopic_Paginated } from ".";
...
interface ProfileAreasInterest {
  ...
  selected: Selected[];
  setSelected: (vals: Selected[]) => void;
}

const AreasInterestModal = ({
  areas,
  selected,
  setSelected,
}: ProfileAreasInterest) => {
    ...
    <Box>
      <Stack spacing={"5"} mt="2">
        ...
        >
          ...
          {selected.length === 0 ? (
            <Box color="gray.400" mt="5" fontSize={"sm"}>
              Please select at least 3 subtopics.
            </Box>
          ) : (
...
export default AreasInterestModal;
